...
namespace GitLabApiClient
{
    ...
    public sealed class MergeRequestsClient
    {
        ...
        /// <param name="options">Merge requests retrieval options.</param>
        /// <returns>Merge requests satisfying options.</returns>
        public async Task<MergeRequest> GetChangesAsync(ProjectId projectId, int mergeRequestId) =>
            await _httpFacade.Get<MergeRequest>($"projects/{projectId}/merge_requests/{mergeRequestId}/changes");
        ...
        /// <param name="options">Merge requests retrieval options.</param>
        /// <returns>Merge requests satisfying options.</returns>
        public async Task<MergeRequest> ApproveAsync(ProjectId projectId, int mergeRequestId) =>
            await _httpFacade.Post<MergeRequest>($"projects/{projectId}/merge_requests/{mergeRequestId}/approve");

        public async Task<MergeRequest> AwardEmoji(ProjectId projectId, int mergeRequestId) =>
            await _httpFacade.Post<MergeRequest>($"projects/{projectId}/merge_requests/{mergeRequestId}/award_emoji?name=thumbsup");

        public async Task<ApprovalState> GetApprovalStateAsync(ProjectId projectId, int mergeRequestId) =>
            await _httpFacade.Get<ApprovalState>($"projects/{projectId}/merge_requests/{mergeRequestId}/approval_state");
        /// <summary>
